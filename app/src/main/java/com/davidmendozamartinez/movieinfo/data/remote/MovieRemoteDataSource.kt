...
class MovieRemoteDataSource(private val service: TheMovieDBService) {
    suspend fun getPopularMovies(): List<MovieDomain> = service.getPopular().map { it.toDomain() }

    suspend fun getMovieDetails(id: Int): MovieDomain = service.getDetails(id).toDomain()
}